...
2009 	& M\"{u}ller - Two-phase approach with hybridization, which the first phase includes \gls{ifs} and Conflict-based Statistics, and the second phase is composed of \gls{hc}, \gls{gd} and optionally \gls{sa}\\
		...
		& Gogos et al. - Two phase approach, which the first phase consists on using a greedy randomized positioning with backtrack mechanism, and the second phase utilizes local search (\gls{sa}), shaking and \gls{ip}\\
