...
namespace Business
{
    public class PeriodHardConstraints
    {
        ...
        {
            return period_hard_constraints_repo.GetAll().Where(p => p.type == type);
        }
        public IEnumerable<PeriodHardConstraint> GetByTypeWithExamId(PeriodHardConstraint.types type, int exam_id)
        {
            return period_hard_constraints_repo.GetAll().Where(p => p.type == type && (p.ex1 == exam_id || p.ex2 == exam_id));
        }
    }
}
