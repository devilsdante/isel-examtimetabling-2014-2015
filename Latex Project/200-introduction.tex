...
\section{Tourist Guides: State of the Art}
\label{sec:sota}
...
%%%%%%%%%%%%%%%%%%%%
\subsection{TouristEye}
%%%%%%%%%%%%%%%%%%%%
One of the most relevant services is the TouristEye~\cite{TouristEyeWeb} and it is available as a Web application, with mobile clients for iPhone~\cite{TouristEyeiPhone} and Android~\cite{touristEyeAndroid}. The TouristEye service offers a wide range of points of interest organized by categories (attractions, restaurants, entertainment, among others). Registered users have the possibility to mark touristic places as visited and leave a comment by indicating their degree of satisfaction (\emph{I love it}, \emph{I like it}, \emph{It's ok} or \emph{I didn't like it}). In addition, users have the possibility to write down their travel journal by taking notes and photos exclusively in the mobile application.\\
\\
In order to visualize points of interest with accurate perspective, the Google Maps~\cite{GoogleMaps} service is integrated within the Web and mobile applications. Users can plan their trips, which are composed by various points of interest, whereas the map service is used to display routes between these locations.\\
\\
%\marginpar{} %%%
The core functionalities of the TouristEye application (iPhone version) are shown on Figure~\ref{fig:touristEyeiPhoneScreenshots}. \\
\begin{figure}[!ht]
        \centering
        \begin{subfigure}[b]{0.25\textwidth}
                \centering
                \includegraphics[height=7.0cm]{./images/screenshots/screenshot_tourist_eye_1.jpg}
                \caption{Exploration of the places near user location.}
                \label{fig:touristEyeNearBy}
        \end{subfigure}%
        \quad\quad\quad\quad
        \begin{subfigure}[b]{0.25\textwidth}
                \centering
                \includegraphics[height=7.0cm]{./images/screenshots/screenshot_tourist_eye_2.jpg}
                \caption{Preview of the suggested location.}
                \label{fig:touristEyeSuggestedLocations}
        \end{subfigure}
        \caption{TouristEye~\cite{TouristEyeWeb} iPhone application screenshots.}
        \label{fig:touristEyeiPhoneScreenshots}
\end{figure}\\
With this application, users have the possibility to list nearby, popular, and recommended places as illustrated on Figure~\ref{fig:touristEyeiPhoneScreenshots}~(\subref{fig:touristEyeNearBy}). The screenshot on Figure~\ref{fig:touristEyeiPhoneScreenshots}~(\subref{fig:touristEyeSuggestedLocations}) illustrates the view where popular locations are suggested to the user. By choosing the \textbf{Add to wishlist} option, the location is added to the list of places that the user would like to visit. Later, this information is used to recommend new places. The recommendation engine is using a mix of Item-Based and User-Based Collaborative Filtering algorithms~\cite{touristEyeRecommender}.

\subsection{GuidePal Offline City Guides}
